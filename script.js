...
function normalizeState(raw = {}) {
  ...
  s.advanceApproved = s.advanceApproved && typeof s.advanceApproved === "object" ? s.advanceApproved : {};
...
function renderHUD() {
  ...
  document.querySelectorAll("[data-progress-text]").forEach((t) => {
    t.textContent = `${completed}/${total} fases completadas`;
  });
  ...
  if (badge) {
    badge.textContent = s.studentName
  ...
  document.querySelectorAll("[data-phase-card]").forEach((card) => {
    ...
    const exhaustedPending = !passed && !completedPhase && (s.attempts[id] || []).length >= MAX_ATTEMPTS;
    if (st) {
      if (passed) {
      ...
      } else if (exhaustedPending) {
        st.textContent = "Pendiente validación";
        st.className = "phase-status warn";
      } else if (accessible) {
...
function renderVerification(id) {
  ...
  box.innerHTML = `
    <section class="panel game-panel">
      ...
      <div class="verify-row">
        <input type="password" id="verify-input" placeholder="********" autocomplete="off" ${done ? "disabled" : ""}>
  ...
  if (!done) {
    ...
    document.getElementById("verify-btn").onclick = submitVerification;
    ...
    <section class="panel game-panel">
      <div class="test-head">
        <div>
          ...
          <p>Puedes revisar todas las preguntas y la respuesta correcta sin registrar intentos.</p>
        </div>
...
function renderTest(id) {
  ...
  if (s.completed[id]) {
    ...
    box.innerHTML = `
      <section class="panel game-panel">
        <h2>${s.passed[id] ? "Fase superada" : "Fase validada"}</h2>
        <p>
          ${s.passed[id]
            ? `Has alcanzado el 80% o más. Mejor resultado: <strong>${result.percent ?? s.percent[id] ?? 0}%</strong>.`
            : `La fase queda registrada como <strong>no superada</strong>. Mejor resultado: <strong>${result.percent ?? s.percent[id] ?? 0}%</strong>.`}
        </p>
        <p>${s.passed[id] ? "Ya puedes continuar la aventura." : "Tras la validación docente, ya puedes avanzar a la siguiente fase."}</p>
        ${nextPhaseButton(id)}
      </section>
    `;
    return;
  ...
  if ((s.attempts[id] || []).length >= MAX_ATTEMPTS) {
    const result = s.results[id] || {};
    box.innerHTML = `
      <section class="panel game-panel">
        <h2>Intentos agotados</h2>
        <p>No has alcanzado el 80% de aciertos. Tu mejor resultado ha sido <strong>${result.percent ?? s.percent[id] ?? 0}%</strong>.</p>
        <p>Revisa la corrección y solicita la validación del docente para poder avanzar a la siguiente fase.</p>
      </section>
    `;
    return;
  }
...
function renderSummary(id) {
  ...
  const result = s.results[id] || {};
  const exhaustedPending = attempts.length >= MAX_ATTEMPTS && !s.passed[id] && !s.completed[id];
  ...
  if (s.passed[id]) {
    ...
    html += `<div class="banner warn">No se ha superado la fase tras ${MAX_ATTEMPTS} intentos. Se registra el mejor resultado (${result.percent ?? s.percent[id] ?? 0}%). Para avanzar a la siguiente misión necesitas validación docente.</div>`;
  } else if (s.completed[id]) {
    html += `<div class="banner warn">Fase cerrada con resultado no superado y validación docente registrada.</div>${nextPhaseButton(id)}`;
  } else if (remaining > 0) {
    html += `<div class="banner warn">Todavía no alcanzas el 80%. Te queda ${remaining} intento(s). Puedes volver a intentarlo.</div>`;
  }

  if (attempts.length >= MAX_ATTEMPTS && !s.passed[id]) {
    attempts.forEach((a, idx) => {
      html += `<div class="correction"><h3>Revisión del intento ${idx + 1}</h3>`;
  ...
  if (exhaustedPending) {
    html += `
      <section class="unlock-panel">
        <h3>Validación para avanzar</h3>
        <p>Después de revisar la corrección, el docente puede validar el avance para pasar a la siguiente fase. El resultado quedará registrado como no superado.</p>
        <div class="verify-row">
          <input type="password" id="unlock-input-${id}" placeholder="********" autocomplete="off">
          <button class="btn pixel-btn" id="unlock-btn-${id}">Validar avance</button>
        </div>
        <p class="mini" id="unlock-msg-${id}"></p>
      </section>
    `;
  }
  ...
  if (exhaustedPending) {
    const submitUnlock = () => {
      const input = document.getElementById(`unlock-input-${id}`);
      const msg = document.getElementById(`unlock-msg-${id}`);
      const val = String(input.value || "").trim();
      if (val === TEACHER_CODE) {
        const st = loadState();
        st.advanceApproved[id] = true;
        st.completed[id] = true;
        st.results[id] = {
          status: "No superado",
          passed: false,
          score: st.best[id] || 0,
          percent: st.percent[id] || 0
        };
        saveState(st);
        renderHUD();
        renderTest(id);
    ...
    document.getElementById(`unlock-btn-${id}`).onclick = submitUnlock;
    document.getElementById(`unlock-input-${id}`).addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        submitUnlock();
      }
...
function renderCertificate() {
  ...
  const rows = APP.phases
    .map((p) => {
      ...
      return `<tr><td>${p.title}</td><td>${result.status}</td><td>${result.percent}%</td><td>${s.verified[p.id] ? "Sí" : "No"}</td><td>${s.passed[p.id] ? "Sí" : "No"}</td></tr>`;
    })
    .join("");
  ...
  panel.innerHTML = `
    <section class="panel game-panel">
      <div class="cert-sheet">
        ...
        <p>ha completado el itinerario por fases, quedando registradas las prácticas validadas, el porcentaje alcanzado en cada test y si la fase quedó superada o no superada.</p>
